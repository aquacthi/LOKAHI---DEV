...
import BUSINESSUNIT_FIELD from '@salesforce/schema/BC_Request__c.Business_Unit__c';
...
export default class BIMNewRequestForm extends NavigationMixin(LightningElement) {
    ...
    @wire(getPicklistValues,
        {
            recordTypeId:"$recordTypeIdBIM360Amazonian",
            fieldApiName: BUSINESSUNIT_FIELD
        }
    )
    wiredBusinessUnits({ error, data }) {
        if (data) {
            this.businessUnitValues = data.values;
            this.error = undefined;
        } else if (error) {
            this.error = error;
            this.businessUnitValues = undefined;
        }        
    }
    ...
    get amazonRequestOptions() {
        return [
            ...
            { label: 'BIM360 Construction Cloud Access', value: 'BIM360' },
    ...
    handleAmazonNextClick() {
        ...
        else if(this.amazonRequestValue == 'BIM360') {
            this.requesttype="BIM";
            this.title="BIM360 Construction Cloud Request";
            this.formtypeselected=true;
        }
    ...
    handleSubmitClick(event) {
       /* this.template.querySelectorAll('lightning-input').forEach(element => {
        ...
        // no project check
        const hasNoProjects = this.projectValues.length == 0 && this.newProjectValues.length == 0;
        if (hasNoProjects == true && this.title != "Token Flex License Request") {
            const event = new ShowToastEvent({
                "title"     : "Unable to submit form",
                "message"   : "Please populate BIM 360 Facility Name or Project Name.",
                "variant"   : "error"
            });
            this.dispatchEvent(event);  
        } 
        // create request if criterion is met
        else if (isInputsCorrect && isInputsCorrect==true && isComboboxesCorrect && isComboboxesCorrect == true) {
            var rec = {
            ...
            } else if (this.title == "BIM360 Construction Cloud Request") {
    ...
    changeFieldHandler(event) {
        ...
        }else if(event.target.dataset.id === 'businessUnit'){
            this.businessUnit = value;
            // handle businessunit change
            let key = this.roles.controllerValues[event.target.value];
            this.roleValues = this.roles.values.filter(opt => opt.validFor.includes(key));
